...
VALID_KEY_CHARS = string.ascii_uppercase \
...
def load_environment_file(envfile, key_length=64):
    ...
    with lock:
        if not os.path.exists(envfile):
            ...
            config['django']['secret_key'] = get_random_string(
                key_length,
                VALID_KEY_CHARS
                )
        ...
        if (os.stat(envfile).st_mode & 0o777) != 0o600:
            raise FilePermissionError(
                "Insecure environment file permissions for %s!" % envfile
                )
        ...
        for key, value in config.items('django'):
            ...
            if envname not in os.environ: 
                os.environ[envname] = re.sub(r'\'|\"', '', value)

        for key, value in config.items('twitter'):
            envname = 'TWITTER_%s' % key.upper() 
            if envname not in os.environ: 
                os.environ[envname] = re.sub(r'\'|\"', '', value)
